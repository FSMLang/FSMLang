...
void  UFMN(eShared_dt)(FSM_DATA_PTR pfsm_data, PARENT_DATA_TYPE_PTR pparent_data)
{
	DBG_PRINTF("%s\n", __func__);

	pfsm_data->cp = pparent_data->u.bop;
	print_machine_data(pfsm_data);
}
...
{
	DBG_PRINTF("subMachine1.data.cp: %s"
          , pmd->cp
         );
}
