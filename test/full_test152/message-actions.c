...
void UFMN(copy_current_message)(FSM_DATA_PTR pfsm_data, PARENT_DATA_TYPE_PTR pparent_data)
{
    DBG_PRINTF("%s", __func__);

	pfsm_data->current_msg = pparent_data->current_msg;

}

void UFMN(init_data)(FSM_DATA_PTR pfsm_data, PARENT_DATA_TYPE_PTR pparent_data)
{
    DBG_PRINTF("%s", __func__);

	pfsm_data->pqueue = &pparent_data->queue;
