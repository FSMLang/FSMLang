   ...
   if (pid_info->name && strlen(pid_info->name))
   {
      ...
      if (pich->pmi->modFlags & mfActionsReturnVoid)
      {
         fprintf(pich->pcmw->cFile
                 , "void %s_%s(p%s) __attribute__((weak));\n"
                 , pich->pmi->name->name
                 , pid_info->name
                 , pich->cp
                );
      ...
      else
      {
         ...
         {
            fprintf(pich->pcmw->cFile
                    , "%s_STATE %s_%s(p%s) __attribute__((weak));\n"
                    , pich->cp
                    , pich->pmi->name->name
                    , pid_info->name
                    , pich->cp
         ...
         {
            fprintf(pich->pcmw->cFile
                    , "%s_EVENT %s_%s(p%s) __attribute__((weak));\n"
                    , pich->cp
                    , pich->pmi->name->name
                    , pid_info->name
                    , pich->cp
...
static bool declare_state_only_transition_functions_for_when_actions_return_states(pLIST_ELEMENT pelem, void *data)
{
   ...
   fprintf(pich->pcmw->cFile
           , "%s_STATE %s_transitionTo%s(p%s) __attribute__((weak));\n"
           , pich->cp
           , pich->pmi->name->name
           , pid_info->name
           , pich->cp
...
static bool declare_transition_fn_for_when_actions_return_events(pLIST_ELEMENT pelem, void *data)
{
   ...
   fprintf(pich->pcmw->cFile
           , "%s_STATE %s_%s(p%s,%s_EVENT) __attribute__((weak));\n"
           , pich->cp
           , pich->pmi->name->name
           , pid_info->name
           , pich->cp
...
static bool declare_state_only_transition_functions_for_when_actions_return_events(pLIST_ELEMENT pelem, void *data)
{
   ...
   fprintf(pich->pcmw->cFile
           , "%s_STATE %s_transitionTo%s(p%s,%s_EVENT) __attribute__((weak));\n"
           , pich->cp
           , pich->pmi->name->name
           , pid_info->name
           , pich->cp
...
static void            commonSourceStart(pCMachineData, pMACHINE_INFO, char *, bool);
static void            subMachineSourceStart(pCMachineData, pMACHINE_INFO, char *, bool);
...
static void commonSourceStart(pCMachineData pcmw, pMACHINE_INFO pmi, char *cp, bool needNoOp)
{
   pID_INFO pid_info;
   ...
   if (needNoOp)
   {
      ...
      {
         fprintf(pcmw->cFile
                 , "void %s_noAction(p%s) __attribute__((weak));\n"
                 , pmi->name->name
                 , cp
                );
      }
      else
      {
         ...
         {
            fprintf(pcmw->cFile
                    , "%s_STATE %s_noAction(p%s) __attribute__((weak));\n\n"
                    , cp
                    , pmi->name->name
                    , cp
                   );
         ...
         {
            fprintf(pcmw->cFile
                    , "%s_EVENT %s_noAction(p%s) __attribute__((weak));\n\n"
                    , cp
                    , pmi->name->name
                    , cp
                   );
   ...
   {
      fprintf(pcmw->cFile
              , "void %s_%s(p%s,%s_STATE) __attribute__((weak));\n\n"
              , pmi->name->name
   ...
   {
      fprintf(pcmw->cFile
              , "%s_STATE %s_noTransitionFn(p%s) __attribute__((weak));\n"
              , cp
              , pmi->name->name
              , cp
             );
   ...
   if (pmi->data)
     fprintf(pcmw->cFile,"void %s_initData(p%s);\n",
       pmi->name->name,
       cp);
   */
...
static void subMachineSourceStart(pCMachineData pcmw, pMACHINE_INFO pmi, char *cp, bool needNoOp)
{
   pID_INFO pid_info;
   ITERATOR_CALLBACK_HELPER ich = { 0 };
...
static int writeCSubMachineInternal(pCMachineData pcmw, pMACHINE_INFO pmi)
{
   ...
   /*
     Source File
   */
   subMachineSourceStart(pcmw, pmi, cp, true);
...
static int writeCMachineInternal(pCMachineData pcmw, pMACHINE_INFO pmi)
{
   ...
   /*
     Source File
   */
   commonSourceStart(pcmw, pmi, cp, true);
...
static int writeCSwitchMachineInternal(pCMachineData pcmw, pMACHINE_INFO pmi)
{
   ...
   */
   ...
   commonSourceStart(pcmw, pmi, cp, false);
