...
static bool print_transitions_only_case(pLIST_ELEMENT,void*);
...
static void writeActionsReturnStateSwitchFSM(pFSMCOutputGenerator pfsmcog)
{
  ...
  {
	  char *param = FDPC(pcmd->cFile, (pmi->states_implemented_by_machine != 0))
					? "pfsm, "
					: pmi->data ? "&pfsm->data, " : ""
					;
	 print_tab_levels(pcmd->cFile,tab_level);
...
static void print_state_fn_epilogue(pCMachineData pcmd, pMACHINE_INFO pmi, pID_INFO pstate, bool transitions_are_possible)
{
        ...
        )
    {
       ...
       if (pmi->states_with_entry_fns_count)
       {
		   param = FDPC(pcmd->cFile, (pmi->states_implemented_by_machine != 0))
					? "pfsm, "
					: pmi->data ? "&pfsm->data, " : ""
					;
           fprintf(pcmd->cFile
...
static bool define_state_returning_state_fn(pLIST_ELEMENT pelem, void *data)
{
	...
	// Are we exiting this state and have things to do?
	if (pich->pcmd->pmi->machineTransition
		|| (pstate->type_data.state_data.state_flags & sfHasExitFn)
		)
	{
		fprintf(pich->pcmd->cFile
				, "\n\tif (retVal != STATE(noTransition))\n\t{\n"
			   );
		...
		if (pich->pcmd->pmi->machineTransition)
		{
			fprintf(pich->pcmd->cFile
					, "\t\tUFMN(%s)(pfsm,retVal);\n"
					, pich->pcmd->pmi->machineTransition->name
				   );
		}
...
static bool print_transitions_only_case(pLIST_ELEMENT pelem, void *data)
{
    /* interpret our data */
    pITERATOR_CALLBACK_HELPER pich       = (pITERATOR_CALLBACK_HELPER) data;
    pID_INFO                  pevent     = (pID_INFO)                  pelem->mbr;
...
{
	set_local_sub_machine_fsm_fn_event_var(pfsmcog->pcmd);
			 ...
			 else
			 {
				 fprintf(pich->pcmd->cFile
						 ...
						 , pich->pcmd->pmi->executes_fns_on_state_transitions
						   ? "new_s" 
						   : "pfsm->state"
						 , ped->psingle_pai->transition->type == STATE ? "STATE" : "UFMN"
						 , ped->psingle_pai->transition->name
...
static bool print_switch_cases_for_events_handled_in_all_states_ars(pLIST_ELEMENT pelem, void *data)
{
   pITERATOR_CALLBACK_HELPER pich  = (pITERATOR_CALLBACK_HELPER) data;
   pID_INFO                  event = (pID_INFO) pelem->mbr;
   pACTION_INFO              pai   = event->type_data.event_data.psingle_pai;

   pEVENT_DATA               ped        = &event->type_data.event_data;
   char                     *s_var = pich->pcmd->pmi->executes_fns_on_state_transitions
	                                 ? "new_s ="
	                                 : "pfsm->state ="
	                                 ;
